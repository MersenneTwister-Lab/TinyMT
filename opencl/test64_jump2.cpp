...
static void status_jump(Buffer& status_buffer,
                        ...
                        int local_num,
                        int total_num);
static void generate_uint64(Buffer& status_buffer,
                            int local_num,
                            int total_num,
                            int data_size);
static void check_data(uint64_t * h_data, int num_data, int total_num);
...
static int test(int argc, char * argv[]) {
    ...
    source = getSource("test64_jump2.cli");
#else
    source = getSource("test64_jump2.cl");
#endif
    std::string option = "-DKERNEL_PROGRAM ";
    bool double_extension = hasDoubleExtension();
    if (double_extension) {
        option += "-DHAVE_DOUBLE ";
    }
#if defined(DEBUG)
    ...
    uint64_t max_data_per_loop
        = getGlobalMemSize() / 2 / total_num / sizeof(uint64_t);
    ...
    while (data_count > 0) {
        generate_uint64(tinymt_status, local_num, total_num, max_data_per_loop);
 ...
 * @param total_num total number of work groups
 * @param step jump steps
 * @return buffer object of jump table
 */
static Buffer make_jump_table_buffer(cl_ulong total_num, uint64_t step)
{
#if defined(DEBUG)
...
/**
 * prepare jump table for the jump in loop
 * @param total_num total number of work groups
 * @param step jump steps
 * @return buffer object of jump table
 */
 ...
 *@param jump_table_buffer buffer of jump_table
 ...
 *@param status_buffer tinymt status in device global memory
 *@param jump_table_buffer for pre-calculated jump table
 *@param local_num number of local work items.
 *@param total_num number of total work items.
 */
static void status_jump(Buffer& status_buffer,
                        Buffer& jump_table_buffer,
                        int local_num,
...
#if defined(DEBUG)
    print_uint64(&output[0], data_size, total_num);
#endif
...
#endif
    tiny->mat1 = TINYMT64J_MAT1;
    tiny->mat2 = TINYMT64J_MAT2;
    tiny->tmat = TINYMT64J_TMAT;
...
static void check_data(uint64_t * h_data, int num_data, int total_num)
{
    ...
    for (j = 0; j < num_data; j++) {
        uint64_t r = tinymt64_generate_uint64(&tiny);
